...
- 懒删除（`Bitmap` 标记）
- 装填因子过高或懒标记过多时自动重哈希
- 未完成：类体未提供构造与析构函数，`ht`、`removed`、`M`、`N`、`L` 均未初始化
...
> **存档状态**：以下命令与用法反映的是项目当初的设计意图，按现状源码直接编译不通过。已知的解析期错误有四处：`utils.h` 第 43、45 行把 `Tuple` 的两个构造函数误写成了 `Entry(...)`；`vector.h` 的 `==` `!=` `<` `>` `+` `-` 六个运算符把 `const` 写在了参数表之前；`dictionary.h` 第 114 行的读取接口用了类模板中未声明的类型 `T`，第 117 行行尾缺分号，第 33 行调用的 `memcpy_s` 是 MSVC 与 C11 Annex K 的接口，libstdc++ 加 glibc 不提供。此外 6 个头文件都没有 include guard，其中 5 个都包含 `utils.h`，同一编译单元内引用两个及以上会因重复定义失败。作为课程存档保留原样，不作修复。
